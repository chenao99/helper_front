...
Page({
  ...
  onLoad: function () {
    var that = this
    wx.request({
      url: 'http://120.26.185.147:10085/CompusSystem/TelNumber',   //服务器地址
      data: {      //请求的参数
      },
      header: {
      ...
      success(res) {
        console.log(res.data.telp);
        that.setData({
          list:res.data.telp,
        })
        wx.hideLoading()
        that.setData({
          logs: that._normalizeSinger(that.data.list)
        })
  ...
  _normalizeSinger(list) {
    ...
    list.forEach((item, index) => {
      ...
      map[key].items.push({
        name: item.teacherName+":     "+item.telNumber,
        avatar: item.img
      })
    })
    // 为了得到有序列表，我们需要处理 map
